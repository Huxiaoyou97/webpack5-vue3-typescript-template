...
import { App } from 'vue';
import { globalPropertiesType } from '@/core/hooks';
...
export function messageSetup(app: App<Element>) {
    gp = {
        ...
        $basePrompt: (content, title, callback1, callback2, options) => {
            ...
            ElMessageBox.prompt(content, title || '温馨提示', p)
                .then(value => {
                    if (callback1) {
                        callback1(value);
                    }
         ...
         * @param {string} cancelButtonText 取消按钮的自定义类名
         */
        $baseConfirm: (content, title, callback1, callback2, confirmButtonText = app.config.globalProperties.$t('确定'), cancelButtonText = app.config.globalProperties.$t('取消')) => {
            ElMessageBox.confirm(content, title || app.config.globalProperties.$t('温馨提示'), {
                confirmButtonText,
                cancelButtonText,
         ...
         */
        $baseNotify: (message, title, type = 'success', position = 'top-right', callback, duration = 3000) => {
            ElNotification({
                title,
                message,
                type,
